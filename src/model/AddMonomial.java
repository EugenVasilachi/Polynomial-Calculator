...
public class AddMonomial {
    public static void addMonomial(Polynomial p, Monomial m) {
        ...
        if(m.getExponent() > p.getPoly().getFirst().getExponent())
            ...
            for(int i = 0; i < p.getPoly().size() && ok == 1; i++){
                if(p.getPoly().get(i).getExponent() == m.getExponent()){
                    Monomial m2 = new Monomial(m.getCoefficient() + p.getPoly().get(i).getCoefficient(), m.getExponent());
                    p.getPoly().set(i, m2);
                    ok = 0;
                }
